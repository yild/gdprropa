...
class CreatePDF extends CreatePDFBase
{
    ...
    public const REPORT_SINGLE_RECORD = 1;
    public const REPORT_FOR_ENTITY = 2;
    public const REPORT_ALL = 3;
    ...
    public function getTabNameForItem(CommonGLPI $item, $withtemplate = 0): bool|string
    {
        ...
        switch ($item->getType()) {
            case Record::class:
                return self::createTabEntry(CreatePDF::getTypeName());
        }
    ...
    public static function displayTabContentForItem(CommonGLPI $item, $tabnum = 1, $withtemplate = 0): bool
    {
        switch ($item->getType()) {
            case Record::class:
                self::showForm(CreatePDF::REPORT_SINGLE_RECORD, $item->fields['id']);
                break;
        }
    ...
    public static function showConfigFormElements($config = []): void
    {
        ...
        if (!isset($config['report_type'])) {
            $config['report_type'] = CreatePDF::REPORT_SINGLE_RECORD;
        }

        if ($config['report_type'] != CreatePDF::REPORT_SINGLE_RECORD) {
            echo "<tr class='tab_bg_1'>";
        ...
        if (CreatePDFBase::isGdprownerPluginActive()) {
            echo "<tr class='tab_bg_1'>";
    ...
    public static function showPrepareForm($report_type): void
    {
        ...
        echo '<div class="center vertical ui-tabs ui-widget ui-widget-content
                    ui-corner-all ui-tabs-vertical ui-helper-clearfix ui-corner-left">';
        echo '<div class="ui-tabs-panel ui-widget-content ui-corner-bottom"
                    aria-live="polite" role="tabpanel" aria-expanded="true" aria-hidden="false">';
    ...
    public static function showForm($report_type, $record_id = -1): void
    {
        ...
        echo "<form name='form' method='GET' action=\"" . $CFG_GLPI['root_doc'] .
            "/plugins/gdprropa/front/createpdf.php\" enctype='multipart/form-data'>";
        ...
        if ($report_type == CreatePDF::REPORT_SINGLE_RECORD) {
    ...
    protected function preparePersonelInfo($person, $caption_not_set, $section_caption): array
    {
        if (
            (
                ...
                !$this->controller_info->fields['users_id_' . $person]
            ) ||
            (!isset($this->controller_info->fields['users_id_' . $person]))
        ) {
    ...
    protected function getControllerName(): string
    {
        if (
            ...
            !empty(trim($this->controller_info->fields['controllername']))
        ) {
    ...
    protected function printHeader(): void
    {
        ...
        if ($this->print_options['show_print_date_time']) {
            $header .= ",   " . sprintf(
                    __("print date/time: %1s", 'gdprropa'),
    ...
    protected function printActivitiesList($records): void
    {
        if ($records) {
            ...
            $this->writeInternal(
                "<h2>" . __(
                    "List of processing activities for which entity deals with personal data",
                    'gdprropa'
                ) . "</h2>",
                ['linebefore' => 8]
            );
            ...
            if (!count($records)) {
                ...
                $tbl = '<table border="1" cellpadding="3" cellspacing="0">' .
                    '<thead><tr>' .
                    '<th width="8%" style="background-color:#323232;color:#FFF;text-align:center;"><h3>' .
                    __("No", 'gdprropa') . '</h3></th>' .
                    '<th width="' . $col_width . '%" style="background-color:#323232;color:#FFF;"><h3>' .
                    __("Description of the activity", 'gdprropa') . '</h3></th>';
                if ($display_introduced_in) {
                    $tbl .= '<th width="50%" style="background-color:#323232;color:#FFF;"><h3>' .
                        __("Introduced in", 'gdprropa') . '</h3></th>';
                }
                $tbl .= '</tr></thead><tbody>';
    ...
    protected function printCoverPage($type, $records, $entities_id = -1): void
    {
        ...
        switch ($type) {
            case CreatePDF::REPORT_SINGLE_RECORD:
                $this->printPageTitle(
                    "<h1><small>" . __("Record of Processing Activities", 'gdprropa')
                    . "</small><br/>" . $records->fields['name'] . "</h1>"
                );
            ...
            case CreatePDF::REPORT_ALL:
                $this->printPageTitle(
                    "<h1><small>" . __("GDPR Records of Processing Activity", 'gdprropa') .
                    "</small><br/>" . '' . "</h1>"
                );
                break;
        }
        ...
        $datas[] = $this->preparePersonelInfo(
            'representative',
            __("Legal representative not set.", 'gdprropa'),
            __("Legal representative", 'gdprropa')
        );
        $datas[] = $this->preparePersonelInfo(
            'dpo',
            __("DPO not set.", 'gdprropa'),
            __("Data Protection Officer", 'gdprropa')
        );
    ...
    protected function addPageForRecord(Record $record, $reporty_type): void
    {
        ...
        if ($reporty_type != CreatePDF::REPORT_SINGLE_RECORD) {
            $this->printPageTitle(
                "<h1><small>" . __("Sheet for GDPR Record of Processing Activities", 'gdprropa') .
                ":</small><br/>" . $record->fields['name'] . "</h1>"
            );
        ...
        */
        ...
        $contracts = 0;
        $contracts += $this->printContracts(
            $record,
            Record_Contract::CONTRACT_JOINTCONTROLLER,
            $contracts
        );
        $contracts += $this->printContracts($record, Record_Contract::CONTRACT_PROCESSOR, $contracts);
        $contracts += $this->printContracts($record, Record_Contract::CONTRACT_THIRDPARTY, $contracts);
        $contracts += $this->printContracts($record, Record_Contract::CONTRACT_INTERNAL, $contracts);
        $contracts += $this->printContracts($record, Record_Contract::CONTRACT_OTHER, $contracts);

        $this->printSoftware($record);
        $this->printSecurityMeasures($record, SecurityMeasure::SECURITYMEASURE_TYPE_ORGANIZATION, true);
        $this->printSecurityMeasures($record, SecurityMeasure::SECURITYMEASURE_TYPE_PHYSICAL);
    ...
    protected function printRecordInformation(Record $record): void
    {
        ...
        $storage_medium = Record::getAllStorageMediumArray();
        $rows[] = [
        ...
        if ($record->fields['pia_required']) {
            $pia_status = Record::getAllPiaStatusArray();
            $rows[] = [
        ...
        if (CreatePDFBase::isGdprownerPluginActive()) {
            if ($this->print_options['show_record_owner']) {
                $owner_info = PluginGdprownerOwner::getOwnerInfo($record->fields['id'], Record::class);
    ...
    protected function printLegalBasisActs(Record $record): void
    {
        ...
        $result = (new Record_LegalBasisAct())
        ...
        if (!count($result)) {
            ...
            $tbl .=
                '<th width="' . $cols_width[3] . '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' .
                __("Content", 'gdprropa') . '</h4></th>';
            if ($this->print_options['show_comments']) {
                $tbl .=
                    '<th width="' . $cols_width[4] . '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' .
                    __("Comment") . '</h4></th>';
            }
            $tbl .=
                '</tr></thead>' .
            ...
            $type = LegalBasisAct::getAllTypesArray();
            ...
            foreach ($result as $item) {
                ...
                if ($this->print_options['show_inherited_from']) {
                    $tbl .=
                        '<td width="' . $cols_width[2] . '%">' .
                        Dropdown::getDropdownName(Entity::getTable(), $dsc->fields['entities_id']) . '</td>';
                }
                $tbl .=
    ...
    protected function printDataSubjectsCategories(Record $record): void
    {
        ...
        $this->writeInternal(
            '<h2>' . Record_DataSubjectsCategory::getTypeName(3) . '</h2>',
            ['linebefore' => 1]
        );
    ...
    protected function printDataRetention(Record $record)
    {
        ...
        if (!count($retention)) {
            ...
            foreach ($retention as $item) {
                $type = Record_Retention::getAllTypesArray();
                $tbl =
                    '<table border="1" cellpadding="3" cellspacing="0">' .
                    '<tbody><tr>' .
                    '<td width="25%" style="background-color:#AFAFAF;color:#FFF;">' .
                    __("Retention regulated by", 'gdprropa') . '</td>' .
                ...
                switch ($item['type']) {
                    case Record_Retention::RETENTION_TYPE_CONTRACT:
                        ...
                        if (!$item['contract_until_is_valid']) {
                            $scale = Record_Retention::getRetentionPeriodScales(
                                $item['contract_retention_scale'],
                                $item['contract_retention_value']
                            );
                            $period = $item['contract_retention_value'] . ' ' . $scale;

                            if ($item['contract_after_end_of']) {
                                $period = sprintf(
                                    __("Data retention: %1\$s after contract is terminated", 'gdprropa'),
                                    $period
                                );
                            }
                        } else {
                        ...
                        $s_names =
                            Record_Contract::getSuppliersNamesNoIds($item['contracts_id'], ', ');
                        if (empty($s_names)) {
                ...
                $tbl .=
                    '</tr><tr>' .
                    '<td width="25%" style="background-color:#AFAFAF;color:#FFF;">' .
                    __("Additional information", 'gdprropa') . '</td>' .
    ...
    protected function printContracts(Record $record, $type, $print_header): int
    {
        ...
        if (!$print_header) {
            $this->writeInternal(
                '<h2>' . __("Contracts related to processing activity", 'gdprropa') . '</h2>',
                [
                    'linebefore' => 1
                ]
            );
        ...
        $subtitle = Record_Contract::getContractTypeStr($type);
        ...
        if (!$number) {
            ...
            $tbl = '<table border="1" cellpadding="3" cellspacing="0">' .
                '<thead><tr>' .
                '<th width="' . $cols_width[0] .
                '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' . __("Supplier") . '</h4></th>' .
                '<th width="' . $cols_width[1] .
                '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' . __("Location") . '</h4></th>' .
                '<th width="' . $cols_width[2] .
                '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' . __("Contact") . '</h4></th>' .
                '<th width="' . $cols_width[3] .
                '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' .
                __("Contract info", 'gdprropa') . '</h4></th>' .
                '<th width="' . $cols_width[4] .
                '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' .
                __("Expiration", 'gdprropa') . '</h4></th>';

            if ($this->print_options['show_comments']) {
                $tbl .=
                    '<th width="' . $cols_width[5] . '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' .
                    __("Comment") . '</h4></th>';
            }
            $tbl .=
                '</tr></thead><tbody>';
            ...
            foreach ($iterator as $data) {
                ...
                $tbl .= '<tr>
                    ...
                    <td width="' . $cols_width[4] . '%" style="background-color:' . $expiry_bkg . '">' . $expiry .
                    '</td>';
                if ($this->print_options['show_comments']) {
                    $tbl .=
    ...
    protected function printPersonalDataCategories(Record $record): void
    {
        ...
        $pdc_list = (new Record_PersonalDataCategory())
            ->find([Record::getForeignKeyField() => $record->fields['id']]);
        ...
        if (!count($pdc_list)) {
            ...
            $tbl .=
                '<thead><tr>' .
                '<th width="' . $cols_width[0] . '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' .
                __("Complete name", 'gdprropa') . '</h4></th>' .
                '<th width="' . $cols_width[1] . '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' .
                __("Special category", 'gdprropa') . '</h4></th>';
            if ($this->print_options['show_inherited_from']) {
                $tbl .=
                    '<th width="' . $cols_width[2] . '%" style="background-color:#AFAFAF;color:#FFF;"><h4>' .
                    __("Introduced in", 'gdprropa') . '</h4></th>';
            ...
            foreach ($pdc_list as $pdc_item) {
                if ($this->print_options['show_full_personaldatacategorylist']) {
                    $sons = getSonsOf(
                        ...
                        $pdc_item['plugin_gdprropa_personaldatacategories_id']
                    );
                } else {
                    $sons = [];
                ...
                foreach ($sons as $son_item) {
                    ...
                    $tbl .=
                        ...
                        '<td width="' . $cols_width[1] . '%" style="text-align:center;">' .
                        Dropdown::getYesNo($pdc->fields['is_special_category']) . '</td>';
                    if ($this->print_options['show_inherited_from']) {
                        $tbl .=
                            '<td width="' . $cols_width[2] . '%">' .
                            Dropdown::getDropdownName(Entity::getTable(), $pdc->fields['entities_id']) . '</td>';
                    }
                    if ($this->print_options['show_comments']) {
                        $tbl .=
    ...
    protected function printSoftware(Record $record): void
    {
        ...
        if (
            ($record->fields['storage_medium'] == Record::STORAGE_MEDIUM_PAPER_ONLY) ||
            ($record->fields['storage_medium'] == Record::STORAGE_MEDIUM_UNDEFINED)
        ) {
            return;
        }
        ...
        if (!count($software_list)) {
            ...
            foreach ($software_list as $item) {
                ...
                if ($this->print_options['show_inherited_from']) {
                    $tbl .=
                        '<td width="' . $cols_width[3] . '%">' .
                        Dropdown::getDropdownName(Entity::getTable(), $item['software_entities_id']) . '</td>';
                }
                if ($this->print_options['show_assets_owners']) {
    ...
    protected function printSecurityMeasures(Record $record, $type, $header = false): void
    {
        ...
        $types = SecurityMeasure::getAllTypesArray();
        $sub_caption = $types[$type];

        if ($header) {
            $this->writeInternal(
                '<h2>' . SecurityMeasure::getTypeName(2) . '</h2>',
                [
                    'linebefore' => 1
                ]
            );
        ...
        $result = $DB->request([
            'FROM' => [
                Record_SecurityMeasure::getTable(),
                SecurityMeasure::getTable()
            ],
            'FKEY' => [
                Record_SecurityMeasure::getTable() => 'plugin_gdprropa_securitymeasures_id',
                SecurityMeasure::getTable() => 'id'
            ],
            'ORDER' => ['name'],
            'AND' => [
        ...
        if (!count($result)) {
            ...
            foreach ($result as $item) {
                ...
                if ($this->print_options['show_inherited_from']) {
                    $tbl .=
                        '<td width="' . $cols_width[2] . '%">' .
                        Dropdown::getDropdownName(Entity::getTable(), $item['entities_id']) . '</td>';
                }
                if ($this->print_options['show_comments']) {
                    $tbl .=
