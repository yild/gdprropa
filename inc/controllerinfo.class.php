...
class PluginGdprropaControllerInfo extends CommonDBChild {
   ...
   static $rightname = 'plugin_gdprropa_controllerinfo';
   ...
   function getTabNameForItem(CommonGLPI $item, $withtemplate = 0) {
      ...
      if (!PluginGdprropaControllerInfo::canView()) {
         return false;
      }
      ...
      switch ($item->getType()) {
         case Entity::class :
            ...
            return self::createTabEntry(PluginGdprropaControllerInfo::getTypeName(), 0);
      }
   ...
   function showForEntity(Entity $entity, $options = []) {
      ...
      if ($this->fields['id'] <= 0 && !PluginGdprropaControllerInfo::canCreate()) {
         echo "<br><br><span class='b'>" . __("Controller information not set.", 'gdprropa') . "</span><br><br>";
      ...
      if ((Session::haveRight('plugin_gdprropa_createpdf', CREATE))) {
         ...
         $_config['report_type'] = PluginGdprropaCreatePDF::REPORT_FOR_ENTITY;
         PluginGdprropaCreatePDF::showConfigFormElements($_config);

         echo "</table>";
         echo "<input type='hidden' name='report_type' value=\"" . PluginGdprropaCreatePDF::REPORT_FOR_ENTITY . "\">";
         echo "<input type='hidden' name='entities_id' value='" . $entity->fields['id'] . "'>";
         echo "<input type='hidden' name='action' value=\"print\">";
         echo "<input type='submit' class='submit' name='createpdf' value='" . __("Create Controller RoPA PDF for Entity", 'gdprropa') . "' />";
         Html::closeForm();
         echo "</div>";
      }
   ...
   static function getFirstControllerInfo($entity_id, $allow_from_ancestors = true) {
      ...
      $controllerinfo = new PluginGdprropaControllerInfo();
      ...
      if (!$allow_from_ancestors) {
         ...
         if ((isset($controllerinfo->fields['id'])) &&
            ...
            foreach (array_reverse($ancestors) as $ancestor) {
               return PluginGdprropaControllerInfo::getFirstControllerInfo($ancestor);
            }
         }
      }
   }
   ...
   static function getContractTypes($entity_id, $compact = false) {
      ...
      $controllerinfo = PluginGdprropaControllerInfo::getFirstControllerInfo($entity_id, PluginGdprropaConfig::getConfig('system', 'allow_controllerinfo_from_ancestor'));
